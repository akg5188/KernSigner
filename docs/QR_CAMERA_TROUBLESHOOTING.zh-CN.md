...
第一次给手机上的小尺寸高密度签名二维码扫码前，必须先调 OV5647 镜头焦距。不要因为手机屏幕上看起来很清楚，就直接判断固件或协议有问题。

照做：

1. 先显示一个普通黑白二维码，不要直接拿 OKX 圆点码或动态签名码调。
2. 开发板和屏幕保持 10-20 cm。
3. 用手指捏住摄像头镜头外圈，轻轻扭动/旋转。
4. 看到二维码边缘、小格子和定位角最清楚时停下。
5. 只扭镜头外圈，不要扭 FFC 排线，不要掰摄像头小板。
6. 普通二维码稳定识别后，再扫 OKX、Bitget、TokenPocket 等小尺寸高密度签名码。

这一步是 ESP32-P4 4.3 的硬件对焦动作，不是软件设置。跳过它，小尺寸高密度码会表现成“完全没反应”或“扫很久才完成”。
...
2. 首次签名扫描前，先按上一节扭动镜头外圈完成对焦。
3. 二维码完整显示，四周留白，不要被按钮、边框、刘海或截图裁边挡住。
4. 开发板和手机尽量平行，不要斜扫。
5. 距离从 10 cm 慢慢拉到 20 cm，找到最清楚的位置。
6. 二维码太小就截图或投屏到电脑，把二维码放大到屏幕中央。
7. 动态码一直差最后几片时，回到钱包重新从第一帧开始扫。
8. 桌面端都扫不到时，先处理图片质量、缩放、曝光和反光。
9. 桌面端能扫、板端扫不到时，再调固件采帧、裁剪、ZBar/quirc 策略。
