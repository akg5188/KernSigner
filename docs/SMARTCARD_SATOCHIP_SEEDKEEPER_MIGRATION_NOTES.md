...
本记录只整理智能卡迁移现状和踩坑边界，不代表商业交付完成，不代表生产安全验收完成。当前阶段仍按“先确认菜单和流程，不编译、不刷机”的节奏推进。
...
- 现阶段不能把 SeedKeeper 管理、写卡、改 PIN、重置、Satochip BTC PSBT/消息签名写成已完成商业能力。
...
- Satochip 只读状态读取。
- SeedKeeper 只读状态读取接口已存在，但仍要和真卡状态页一起验收。
- Satochip Web3 连接码。
...
- SeedKeeper 状态、剩余空间、列表、日志、导入/导出/删除类接口需要真卡逐项验收和失败恢复设计。
- SeedKeeper 写入助记词、保存密码、保存描述符、克隆、从卡加载秘密等涉及秘密材料的流程，需要专门安全设计和 UI 风险提示。
...
- 不能宣称 SeedKeeper 管理、写卡、改 PIN、重置已经完成生产验收。
...
4. 再单独设计 SeedKeeper 只读列表/容量/状态。
5. 最后才评估写卡、PIN 管理、重置、SeedKeeper 导入导出、Satochip BTC 卡签名和证书/2FA/高级维护。
