...
static void success_from_kef_decrypt(const uint8_t *data, size_t len) {
  ...
  if (!descriptor_str) {
    kef_decrypt_page_destroy();
    dialog_show_error(i18n_tr_or("dialog.operation_failed", "Out of memory"),
                      NULL, 0);
    storage_browser_show();
    return;
  }
...
static void load_selected(int idx, const char *filename) {
  ...
  if (ret != ESP_OK) {
    dialog_show_error(i18n_tr_or("dialog.load_failed", "File load failed"),
                      NULL, 0);
    return;
  }
  ...
  if (encrypted) {
    if (!kef_is_envelope(data, data_len)) {
      free(data);
      dialog_show_error(i18n_tr_or("dialog.invalid_input",
...
void load_descriptor_storage_page_create(lv_obj_t *parent,
  ...
  storage_browser_config_t config = {
      .item_type_name = i18n_tr_or("descriptor.descriptor", "Descriptor"),
      .location = location,
