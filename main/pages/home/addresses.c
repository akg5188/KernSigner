...
static void show_address_detail(int index) {
  ...
  snprintf(title, sizeof(title), "%s #%u",
           show_change ? i18n_tr_or("address.change", "Change")
                       : i18n_tr_or("address.receive", "Receive"),
           addr_idx);
...
static void refresh_address_list(void) {
  ...
  if (policy == WALLET_POLICY_MULTISIG && !wallet_has_descriptor()) {
    lv_obj_t *msg = theme_create_label(
        ...
        i18n_tr_or("address.multisig_descriptor_required",
                   "Multisig addresses require a wallet descriptor.\n\n"
                   "Scan a wallet descriptor QR before viewing addresses."),
        false);
...
void addresses_page_create(lv_obj_t *parent, void (*return_cb)(void)) {
  ...
  type_dropdown = theme_create_dropdown(btn_cont, type_options);
