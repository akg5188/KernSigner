#include "krux_services.h"

static const krux_service_status_t service_statuses[] = {
    {KRUX_SERVICE_WALLET, "钱包服务", KRUX_SERVICE_READY,
     "导入、创建、公钥、地址、备份、签名。"},
    {KRUX_SERVICE_CRYPTO, "加密服务", KRUX_SERVICE_READY,
     "签名前检查账户、网络和路径。"},
    {KRUX_SERVICE_QR, "二维码服务", KRUX_SERVICE_READY,
     "支持扫码、导入和签名。"},
    {KRUX_SERVICE_CAMERA, "相机服务", KRUX_SERVICE_READY,
     "相机和识别可用。"},
    {KRUX_SERVICE_STORAGE, "存储服务", KRUX_SERVICE_READY,
     "支持存储卡检查和浏览。"},
    {KRUX_SERVICE_DISPLAY, "显示触摸", KRUX_SERVICE_READY,
     "中文界面、触摸可用。"},
    {KRUX_SERVICE_SMARTCARD, "智能卡检测", KRUX_SERVICE_CANDIDATE,
     "CCID、连接、签名、地址已接入；写卡和改 PIN 暂隐藏。"},
};

const krux_service_status_t *krux_service_status_at(size_t index) {
  if (index >= krux_service_status_count())
    return NULL;
...
const char *krux_service_state_name(krux_service_state_t state) {
  switch (state) {
  case KRUX_SERVICE_READY:
    return "可用";
  case KRUX_SERVICE_CANDIDATE:
    return "可用";
  case KRUX_SERVICE_STUB:
    return "维护";
  case KRUX_SERVICE_BLOCKED:
    return "暂停";
  default:
    return "未知";
  }
